...
$(document).ready(function() {
    ...
    userDataTable = $('#user_datatable').DataTable({
        ...
        "order": [],
        "ajax": {
            url: ajax_url + "training/fetch_training_videos",
            type: "POST",
            dataType: 'json',
            data: function(d) {
                d.Select_Training_Type = $('#Select_Training_Type').val()
            },
        },
        // "oLanguage":
        // {
    ...
    $('#Select_Training_Type').change(function(){
    ...
    $("#add_user").validate({
        ...
        submitHandler: function() {
            ...
            $.ajax(ajax_url + 'training/add_video', {
                data: formData,
                method: 'POST',
                processData: false,
                contentType: false,
...
function edit_user(user_id) {
    $.post(ajax_url + 'training/get_video', {
        'id': user_id
        ...
        if (data.flash_status) {
            ...
            $('#Training_Type').selectpicker('val',data.data.Training_Type);
            $('#Training_Type').selectpicker('refresh');
    ...
    $.confirm({
        ...
        buttons: {
            confirm: {
                ...
                action: function() {
                    if (user_id != '' && status != '') {
                        $.post(ajax_url + 'training/activate_account', {
                            'id': user_id,
                            'status': status
                        }, function(data, textStatus, xhr) {
                            notification(data.flash_title, data.flash_message, data.flash_status);
    ...
    $.confirm({
        ...
        buttons: {
            confirm: {
                ...
                action: function() {
                    if (user_id != '') {
                        $.post(ajax_url + 'training/delete_user', {
                            'id': user_id,
                        }, function(data, textStatus, xhr) {
                            notification(data.flash_title, data.flash_message, data.flash_status);
                            if (data.flash_status) {
...
function load_more(page,training_type){
    if($('.ladda-button').length){
        var l = Ladda.create(document.querySelector('.ladda-button'));
        l.start();
     }
     $.post(ajax_url + 'training/view_trainings', {
...
function default_video(user_id, msg, t_type) {
    msg = msg.split('_').join(' ');
    $.confirm({
        title: 'Confirm!',
        content: msg,
        buttons: {
            confirm: {
                ...
                action: function() {
                    if (user_id != '') {
                        $.post(ajax_url + 'training/default_video', {
                            'id': user_id,
