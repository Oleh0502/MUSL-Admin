...
class Supplier extends MY_Controller
{
    ...
    {
        if ($this->auth->has_permission('manage-supplier'))
        {
            $data['items'] = $this->common_model->get_data('items', array('Is_Blocked'=>'0'));
            $data['packages'] = $this->common_model->get_data('packages', array('Package_For'=>'supplier'));
    ...
    public function fetch_users()
    {
        ...
        $this->load->model("datatables/Supplier_list");
        $fetch_data = $this->Supplier_list->make_datatables($where);
        $data       = array();
        foreach ($fetch_data as $row)
        {
            ...
            $package_details = $this->common_model->get_data('packages_purchase',array('Customer_Id'=>$row->User_Id),'single');
            if($package_details){
                if(date('Y-m-d', strtotime($package_details['Valid_To'])) > date('Y-m-d')){
                    $sub_array[] = '<span class="label label-sm label-success"> Active </span></br><b>Valid Till: </b>'.date('d M, Y', strtotime($package_details['Valid_To'])).'';
                }else{
                    $sub_array[] = '<span class="label label-sm label-danger"> De-Active </span></br><b>Valid Till: </b>'.date('d M, Y', strtotime($package_details['Valid_To'])).'<a data-toggle="modal" type="button" title="Renew Subscription" onclick="do_subscription(' . $row->User_Id . ');" class="label label-success">Renew Subscription</a>';
                }
            }else{
                $sub_array[] = '<span class="label label-sm label-warning" onclick="do_subscription('. $row->User_Id.')"> Not Subscribed Yet </span>';
            }
            ...
            if ($this->auth->has_permission('manage-supplier', 'Edit'))
            {
            ...
            if ($this->auth->has_permission('manage-supplier', 'Delete'))
            {
                if ($row->Is_Blocked == 1)
                {
            ...
            if ($this->auth->has_permission('manage-supplier', 'Perm_Delete'))
            {
            ...
            {
                $buttons .= '<a href="'.base_url('payment/all_transactions/').$row->User_Id.'" title="View Subscription History" class="btn btn-icon-only blue"><i class="fa fa-link"></i></a>';
            }
        ...
        $output = array(
            ...
            "recordsTotal"    => $this->Supplier_list->get_all_data($where),
            "recordsFiltered" => $this->Supplier_list->get_filtered_data($where),
            "data"            => $data,
        );
    ...
    public function add_user()
    {
        if ($this->input->is_ajax_request())
        {
            if ($this->input->post())
            {
                ...
                $perm_check = 0;
                if ($this->auth->has_permission('manage-supplier', 'Add'))
                {
                    $perm_check = 1;
                }                
                if ($perm_check == 0)
	            ...
	            if ($this->common_model->insert_data('users', $input))
	            {
	                ...
	                //Mail to Admin
	                $data['subject'] = $this->lang->line('supplier_reg_subject_for_admin');
	                $data['message'] = $this->lang->line('supplier_reg_body_for_admin');
		            ...
		            $notification['flash_message'] = $this->lang->line('supplier_added');
	            }
            	else
            	{
	                $notification['flash_status']  = false;
    ...
    public function add_subscription()
    {
        if ($this->input->is_ajax_request())
        {
            if ($this->input->post())
            {
                ...
                $this->form_validation->set_rules('Subs_User_Id', 'user id', 'required');
                $this->form_validation->set_rules('Package_Id', 'package', 'required|is_unique[users.User_Email]');
                if ($this->form_validation->run() == false)
                {
                    $notification['flash_status'] = false;
                    $notification['flash_type'] = 'validation';
                ...
                $input['Created_Date'] = date('Y-m-d H:i:s');
                $input['Active'] = 'yes';
                $input['Customer_Id'] = $input['Subs_User_Id'];
                unset($input['Subs_User_Id']);

                $package = $this->common_model->get_data('packages', array('Package_Id'=>$input['Package_Id']),'single');
                $input['Valid_From'] = date('Y-m-d H:i:s');
                $input['Valid_To'] = date('Y-m-d', strtotime("+".$package['Package_Duration']." ".$package['Duration_Type']."", strtotime(date('Y-m-d'))));
                if ($this->common_model->insert_data('packages_purchase', $input))
                {
                    $insert_id = $this->db->insert_id();

                    $this->common_model->update_data('packages_purchase', array('Active'=>'no'),  array('Customer_Id'=>$input['Customer_Id'],'P_Id!='=>$insert_id));
                   
                    $user = $this->common_model->get_data('users',array('User_Id'=>$input['Customer_Id']),'single');

                    //Mail to User
                    $data['subject'] = $this->lang->line('subscription_add_subject_temp');
                    $data['message'] = $this->lang->line('subscription_add_body_temp');
                    $replaceto       = array("userfirstname__","plan__","valid_from__","valid_to__");
                    $replacewith     = array($user['User_First_Name'] . " " . $user['User_Last_Name'],$package['Package_Name'],$input['Valid_From'],$input['Valid_To']);
                    $data['content'] = str_replace($replaceto, $replacewith, $data['message']);
                    $view_content    = $this->load->view('email_template', $data, true);
                    send_email($user['User_Email'], $data['subject'], $view_content);
                   
                    $notification['flash_status']  = true;
                    $notification['flash_type'] = 'account';
                    $notification['flash_title']   = $this->lang->line('success_label');
                    $notification['flash_message'] = $this->lang->line('subscription_done');
                }
                else
                {
                    $notification['flash_status']  = false;
    ...
    public function edit_user()
    {
        if ($this->input->is_ajax_request())
        {
            if ($this->input->post())
            {
                ...
                $update_data['Business_Category'] = $input['edit_Items'];
	            ...
	            if ($this->common_model->update_data('users', $update_data,  array('User_Id'=>$input['User_Id'])))
	            {
		            ...
		            $notification['flash_type'] = 'account';
		            $notification['flash_title']   = $this->lang->line('success_label');
		            $notification['flash_message'] = $this->lang->line('supplier_updated');
	            }
            	else
            	{
	                $notification['flash_status']  = false;
    ...
    public function delete_user()
    {
        if ($this->input->post())
        {
            ...
            if ($this->common_model->update_data('users', $data, $where))
            {
                $notification['flash_status']  = true;
                $notification['flash_title']   = $this->lang->line('success_title');
                $notification['flash_message'] = $this->lang->line('supplier_deleted');
            }
            else
            {
                $notification['flash_status']  = false;
